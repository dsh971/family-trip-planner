...
describe("PlaceCard photo hero", () => {
  ...
  it("synthetic wg: place (photoReference = null) shows placeholder, not img", () => {
    // wg: places always have null photoReference per the plan
...
describe("PlaceCard description snippet", () => {
  ...
  it("empty string description is treated as truthy (non-null) — renders", () => {
    // Empty string is non-null; rendering guard is `place.description && ...`
    // In React, empty string renders nothing via &&, but the decision function
    // only checks for null per the spec. An empty string is non-null.
